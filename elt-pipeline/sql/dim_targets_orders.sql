...
INSERT INTO analytics.dim_targets_orders (order_sk, customer_sk, customer_id, ontime_target, infull_target, otif_target, run_id, load_time, is_current)
SELECT
    ...
    CURRENT_TIMESTAMP(),
    TRUE AS is_current
FROM raw1.targets_orders o
LEFT JOIN analytics.dim_targets_orders d
  ON o.customer_id = d.customer_id
 AND d.is_current = TRUE
WHERE o.run_id = '{{ run_id }}'
  AND NOT EXISTS (                     
    SELECT 1 FROM analytics.dim_targets_orders d
